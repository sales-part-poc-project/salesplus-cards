...
- `scripts/cards_data.py` — 적재(`load_local`·`load_remote`)와 스키마 검증. `.github-private/scripts/update_cards.py` 와 같은 규칙이다. **둘을 같이 고친다.** (업무 요약 카드 `daily.json` 과 파트 일정의 "오늘 제외" 앵커·`done` 취소선은 이 사이트에만 있다, 2026-09-17)
...
- 섹션 순서는 업무 요약(최근 2일) → 파트 일정(내일·모레) → 프로젝트 → 멤버 → 최근 변경. 파트 일정 창은 `schedule_anchor(today)` 부터 — 오늘 일은 업무 요약이 말한다.
