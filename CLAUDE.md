...
- `scripts/cards_data.py` — 적재(`load_local`·`load_remote`)와 스키마 검증. `.github-private/scripts/update_cards.py` 와 같은 규칙이다. **둘을 같이 고친다.** (업무 요약 카드 `daily.json` 과 파트 일정의 `done` 취소선, 네트워킹비 카드 `budget.json` 은 이 사이트에만 있다, 2026-09-17 · 09-22)
...
- 섹션 순서는 업무 요약(최근 2일) → 파트 일정(오늘·내일) → 프로젝트 → 멤버 → 네트워킹비 → 최근 변경. 네트워킹비는 사람별 할당·사용·잔액 숫자만 그린다 (`validate_budget` 이 잔액 = 할당 − 사용을 다시 확인한다). 파트 일정 창은 `schedule_anchor(today)`(오늘) 부터 업무일 2일 — 오늘 처리한 일은 `done` 취소선으로 남긴다 (2026-09-22).
